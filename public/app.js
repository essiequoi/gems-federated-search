...
function getSeriesCard(hit) {
  ...
  const image =
    firstValue(hit, FIELD_MAP.series.image) ||
    "https://via.placeholder.com/500x300?text=Series";
...
function getProductRow(hit) {
  ...
  const description = truncate(
    firstValue(hit, FIELD_MAP.products.description) || "",
    180
  );
      ...
      ${
        image
          ? `<img src="${escapeHtml(image)}" alt="${escapeHtml(
              title
            )}" style="width:120px;height:120px;object-fit:contain;margin-bottom:12px;" />`
          : ""
      }
      <h3>${escapeHtml(title)}</h3>
      <p>${escapeHtml(description)}</p>
...
function getDocumentRow(hit) {
  const title =
    firstValue(hit, FIELD_MAP.documents.title) || "Untitled document";
  const description = truncate(
    firstValue(hit, FIELD_MAP.documents.description) || "",
    220
  );
...
function getPageRow(hit) {
  ...
  const description = truncate(
    firstValue(hit, FIELD_MAP.pages.description) || "",
    220
  );
...
function updateTabCounts() {
  document.getElementById("count-series").textContent =
    getIndexHitCount("series");
  document.getElementById("count-products").textContent =
    getIndexHitCount("products");
  document.getElementById("count-documents").textContent =
    getIndexHitCount("documents");
  document.getElementById("count-pages").textContent =
    getIndexHitCount("pages");
}
...
function updateResultsMeta() {
  ...
  const start =
    results.nbHits === 0 ? 0 : results.page * results.hitsPerPage + 1;
  const end = Math.min(
